...
import com.huanmedia.ilibray.utils.io.Charsets;
...
import retrofit2.Converter;
 ...
 final class GsonResponseBodyConverter<T> implements Converter<ResponseBody, T> {
  ...
  private GsonConverterFactory.OutLoginListener mListener;

  public void setListener(GsonConverterFactory.OutLoginListener listener) {
    mListener = listener;
  }
    ...
    try {
      ...
      if (object instanceof DataResponse){
        ...
        if (httpStatus.isCodeInvalid()) {
          ApiException exception = new ApiException(httpStatus.getCode(), httpStatus.getMessage());
          if (mListener!=null){
            switch (httpStatus.getCode()){//异常下线
              case -8210:
              case -8211:
              case -8212:
                mListener.outlogin(exception);
            }
          }
          value.close();
          throw exception;
