...
public class AppValidationMgr {
	...
	//手机号表达式
	private final static Pattern phone_pattern = Pattern.compile("^(13|15|18|17|16|19)\\d{9}$");
...
//	private final static Pattern username_pattern = Pattern.compile("^[A-Za-z0-9_]{1}[A-Za-z0-9_.-]{3,31}");
	private final static Pattern username_pattern = Pattern.compile("^[A-Za-z0-9_ \u0391-\uFFE5]{1}[ \u0391-\uFFE5 A-Za-z0-9_.-]{0,20}");
...
//^(?=.*\\d)(?=.*[a-z])(?=.*[A-Z]).{8,10}$
	...
	public static boolean isIDCard(String IDCardNo) {
		...
		//================ 地区码时候有效 ================
		Hashtable hashtable = IDCardInfoMgr.getAreaCodeAll();
	...
	/**
	 * 判断是否为用户名账号(规则如下：用户名由下划线或字母开头，由中文、数字、字母、下划线、点、减号组成的1-20位字符)
	 * @param username 用户名 
	 * @return boolean   
	 */
