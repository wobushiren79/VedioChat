...
public class WaveLoadingView extends View {
    ...
    private static final int DEFAULT_WAVE_COLOR_TYPE=0;//Default colorType
...
//        RECTANGLE
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        // Init ColorType
        mColorType = attributes.getInteger(R.styleable.WaveLoadingView_wlv_color_type, DEFAULT_WAVE_COLOR_TYPE);
        if (mColorType==1){
            mStartColor=attributes.getColor(R.styleable.WaveLoadingView_wlv_gradient_startColor, DEFAULT_WAVE_COLOR);
            mEndColor =attributes.getColor(R.styleable.WaveLoadingView_wlv_gradient_endColor, DEFAULT_WAVE_COLOR);
        }
        // Init Wave
        ...
        // Init Border
        mBorderDistance=attributes.getDimension(R.styleable.WaveLoadingView_wlv_border_distance,dp2px(4));
        ...
        if (mWaveShader != null) {
            ...
            switch (mShapeType) {
                ...
                case 1:
                    ...
                    // Draw background
                    canvas.drawCircle(getWidth() / 2f, getHeight() / 2f, radius-mBorderDistance, mWaveBgPaint);
                    canvas.drawCircle(getWidth() / 2f, getHeight() / 2f, radius-mBorderDistance, mWavePaint);
                    break;
                // Draw square
                case 2:
                    if (borderWidth > 0) {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (mColorType==1){
            mColorShader = new LinearGradient(0, h/2, w, h/2, mStartColor,mEndColor,
                    Shader.TileMode.CLAMP);
            mBorderPaint.setShader(mColorShader);
            mWaveBgPaint.setShader(mColorShader);
        }
