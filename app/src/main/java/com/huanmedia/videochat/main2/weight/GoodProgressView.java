...
//  private RectF rect;//绘制范围
        ...
        for (int i = 0; i < n; i++) {
            ...
            switch (attr) {
                ...
                case R.styleable.GoodProgressView_textColor:
                    ...
                    break;
                case R.styleable.GoodProgressView_progress:
                    // 文字颜色，默认设置为灰色
                    progressValue = a.getInt(attr, 0);
                    break;
                case R.styleable.GoodProgressView_maxProgress:
                    // 文字颜色，默认设置为灰色
                    maxProgress = a.getInt(attr, 0);
                    break;
                case R.styleable.GoodProgressView_showProgressText:
                    // 文字颜色，默认设置为灰色
                    mShowProgressText = a.getBoolean(attr, true);
                    break;
            }
        }
        a.recycle();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //按比例计算进度条各部分的值
        float unit = Math.min(((float) mWidth) / 200, ((float) mHeight) / 20);
        float lineWidth = 3 * unit;//线粗
        float innerCircleDiameter = 3 * unit;//内圆直径
        float outerCircleDiameter = 6 * unit;//外圆直径
        float wordHeight = (12*(mShowProgressText?1:0)) * unit;//字高//9*unit
//      float wordWidth = 26*unit;//字长
        float offsetLength = 3* unit;//留空
//      float width = 300*unit;//绘画区域的长度
        float height = 40 * unit;//绘画区域的高度
        float progressWidth = (146+(mShowProgressText?0:42)) * unit;//绘画区域的长度
        ...
        float offsetHeight = mHeight / 2;
        float offsetWidth = offsetLength;

        float section = ((float) progressValue) / maxProgress;
        if (section > 1)
            section = 1;
...
//                    offsetWidth + section * progressWidth - 1 * unit, offsetHeight - (float) 3 * unit, mPaint);
//
//            canvas.drawLine(offsetWidth + section * progressWidth - 6 * unit, offsetHeight + (float) 1.5 * unit,
//                    offsetWidth + section * progressWidth - 1 * unit, offsetHeight + (float) 3 * unit, mPaint);
//        }
        ...
        if (mShowProgressText){
            ...
            mPaint.setTextSize(wordHeight+2);
            //计算坐标使文字居中
            FontMetrics fontMetrics = mPaint.getFontMetrics();
            float fontHeight = fontMetrics.bottom - fontMetrics.top;
            float baseY = offsetHeight + fontHeight / 2 - fontMetrics.bottom;
            float offsetText=mPaint.measureText(progressValue + "/");
            canvas.drawText( progressValue + "/", progressWidth + 3.2f * offsetWidth, baseY, mPaint);//略微偏下，baseline
            mPaint.setTextSize(wordHeight-2);
            mPaint.setTypeface(Typeface.DEFAULT);
            canvas.drawText(""+maxProgress, progressWidth + 3.2f * offsetWidth+offsetText, baseY, mPaint);//略微偏下，baseline
        }
