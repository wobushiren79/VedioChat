...
import com.huanmedia.ilibray.utils.data.namegenerator.Md5FileNameGenerator;
...
public class Luban implements Handler.Callback {
  ...
  private Md5FileNameGenerator mMd5Name;
  ...
  private Luban(Builder builder) {
    ...
    this.mMd5Name= new Md5FileNameGenerator();
  }
   ...
   *     A context.
   */
  private File getImageCacheFile(Context context, String original,String suffix) {
    if (TextUtils.isEmpty(mTargetDir)) {
      mTargetDir = getImageCacheDir(context).getAbsolutePath();
    }
    String cacheBuilder = mTargetDir + "/" +
           mMd5Name.generate(original)+ (TextUtils.isEmpty(suffix) ? ".jpg" : suffix);
          ...
          @Override public void run() {
            try {
              ...
              File result = Checker.isNeedCompress(mLeastCompressSize, path) ?
                  new Engine(path, getImageCacheFile(context,path, Checker.checkSuffix(path))).compress() :
                  new File(path);
  ...
  @WorkerThread private File get(String path, Context context) throws IOException {
    return new Engine(path, getImageCacheFile(context,path, Checker.checkSuffix(path))).compress();
  }
  ...
  @WorkerThread private List<File> get(Context context) throws IOException {
    ...
    while (iterator.hasNext()) {
      ...
      if (Checker.isImage(path)) {
        results.add(new Engine(path, getImageCacheFile(context,path, Checker.checkSuffix(path))).compress());
      }
      iterator.remove();
    }
